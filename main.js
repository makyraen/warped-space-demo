...
const CONFIG = {
    planeSize: 1000,
    maxMassCount: 5,
    // 200 → 반지름 45, M=10, 단일 질량 우물 깊이 약 194.
    // 한때 "우물이 깊어지면 격자가 사라진다"는 이유로 100까지 낮췄었지만, 진짜 원인은
    // 프래그먼트 셰이더의 깊이 포화였고 depthColorScale로 고쳤다(아래). 그래서 원래 값으로
    // 되돌렸다 — 이 상한은 이제 렌더링 제약이 아니라 조작감의 문제다.
    maxMassValue: 200,
    minMassValue: 20,
...
// 측정·검증용 훅. `?debug` 쿼리가 있을 때만 열린다(일반 실행에는 노출되지 않는다).
