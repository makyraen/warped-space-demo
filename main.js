...
const state = {
    viewMode: 'GOD', isSpawning: false, isCharging: false, chargeStartTime: 0, masses: [], spawnsInFlight: 0,
...
function createMass(pos, scale, mass) {
    ...
    // 바운스 애니메이션이 끝나기 전에도 개수/상한에 즉시 반영되도록 in-flight로 집계
    state.spawnsInFlight++;
    updateShaderData();
    gsap.to(mesh.position, {
        y: 0, duration: 1.0, ease: "bounce.out",
        onComplete: () => {
            spawnRipple(pos.x, pos.z);
            state.spawnsInFlight = Math.max(0, state.spawnsInFlight - 1);
...
btnReset.addEventListener("click", () => {
    ...
    state.masses.forEach(m => { scene.remove(m.mesh); m.mesh.material.dispose(); });
...
btnDelete.addEventListener("click", () => {
    if(selectedMassForDelete) {
        ...
        if(index > -1) { state.masses.splice(index, 1); scene.remove(selectedMassForDelete); selectedMassForDelete.material.dispose(); updateShaderData(); }
    }
