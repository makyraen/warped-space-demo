...
앱 코드와 함께 사용하는 수치 검증, 성능 측정 및 원자료이다. 현재 논문은 사다리꼴 각도 갱신을 사용하는 앱과 `performance-current.json`의 Windows 측정을 기준으로 한다.

## 준비와 실행

Node.js 20 이상과 Python을 설치한다. 브라우저 측정은 설치된 Edge를 기본으로 사용한다. Chrome을 사용하려면 `WARPED_BROWSER_CHANNEL=chrome`을 설정한다. Three.js와 GSAP를 CDN에서 받으므로 인터넷 연결이 필요하다.
...
| `results/performance-current.json` | Windows, Intel 내장 GPU, headless Edge의 개별 프레임과 통계 |
...
성능 측정은 2모드 × 물체 수 1·3·5 × 5라운드, 조건당 3초로 수행했다. 라운드 평균의 중앙값은 2.79–3.01 ms였으며, 모든 라운드의 프레임을 합친 p95는 최대 17.33 ms였다. 중앙값은 정렬한 가운데 값(짝수 표본은 가운데 두 값의 평균), 분위수는 Type 7 선형 보간을 사용한다. 이 값은 브라우저 콜백 간격이며 GPU 실행시간이나 실제 화면 주사율이 아니다. headless 실행의 GPU 사용 여부는 원자료의 렌더러 문자열로 확인한다.
